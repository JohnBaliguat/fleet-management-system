...
<head>
  ...
  <style>
    .highlight-red {
      background-color: #f8d7da !important; /* Bootstrap light red */
    }
    .manpower-btn {
              ...
              <div class="card">
                <div class="card-body">
                  <div class="d-md-flex align-items-center">
                    <div>
                      <h4 class="card-title">List of Units to Rescue</h4>
                    </div>
                  </div>
                  <!-- Body -->
      ...
      <div class="modal-dialog modal-lg">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title">Dispatch Rescue</h5>
          ...
          <div class="modal-footer">
            ...
            <button class="btn btn-success" id="submitDispatch">Submit Dispatch</button>
          </div>
        </div>
      </div>
    </div>
  ...
  <script>
    $(document).ready(function(){
      let table = $('#table-data').DataTable({
        "ajax": "table-fetch/fetch_rescue.php",
        "columns": [
          { "data": "driver" },
          { "data": "unit" },
          { "data": "start" },
    ...
    updateLiveTimes();
      ...
      $("#submitDispatch").on("click", function () {
    ...
    $(document).on("change", ".manpower-check", function () {
        $(this).closest(".manpower-btn").toggleClass("active", this.checked);
    });
              ...
              data.forEach(mp => {
                  rows += `
                    ...
                    <td class="text-center">
                      ...
                      <td class="mp-name">
                        ${mp.smp_fname} ${mp.smp_mname} ${mp.smp_lname}
                      </td>
                      <td class="mp-role">${mp.smp_role}</td>
    ...
    $(document).on("click", ".forward-btn", function() {
      var r_id = $(this).data("id");
          ...
          Swal.fire({
              title: "Are you sure?",
              text: "This unit will be forwarded to the workshop.",
              icon: "warning",
              ...
              if (result.isConfirmed) {
                  $.ajax({
                      url: "php/forward_to_workshop.php",
                      type: "POST",
                      data: { r_id: r_id },
                      ...
                      error: function() {
                          Swal.fire({
                              icon: "error",
                              title: "Error",
                              text: "Something went wrong while forwarding to workshop.",
      ...
      $(document).on("click", ".good-btn", function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: "php/mark_pending_rescueStat.php",
                    type: "POST",
                    data: { r_id: r_id },
                    success: function(response) {
                        Swal.fire({
