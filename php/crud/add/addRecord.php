...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($stmt->execute()) {
        ...
        $trip1_stmt = $conn->prepare("INSERT INTO trips 
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'Active', ?, ?)");
        $trip1_stmt->bind_param("isssssssssdss", $dispatch_id, $trip1_type, $trip1_costumer, $trip1_container, $trip1_status, $trip1_activity, $trip1_segment, $trip1_hauling_type, $trip1_from, $trip1_to, $trip1_km_run, $d_trailer, $d_genset);
        $trip1_stmt->execute();
        $trip1_stmt->close();
        ...
        if (!empty($trip2_costumer) || !empty($trip2_container) || !empty($trip2_status) || !empty($trip2_segment) || !empty($trip2_from) || !empty($trip2_to)) {
            $trip2_stmt = $conn->prepare("INSERT INTO trips 
                (d_id, trip_type, costumer, trip_container, trip_containerStat, container_activity, trip_haulingSegment, trip_haulingType, trip_from, trip_to, km_run, trip_status, trip_trailer, trip_genset) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'Active', ?, ?)");
            $trip2_stmt->bind_param("isssssssssdss", $dispatch_id, $trip2_type, $trip2_costumer, $trip2_container, $trip2_status, $trip2_activity, $trip2_segment, $trip2_hauling_type, $trip2_from, $trip2_to, $trip2_km_run, $trip2_trailer, $trip2_genset);
            $trip2_stmt->execute();
            $trip2_stmt->close();
            ...
            if (!empty($trip2_trailer)) {
                ...
                $stmt2->bind_param("ssssss", $trip2_trailer, $d_driverName, $location2, $recordedType2, $d_dispatcher, $d_datetime);
                if (!$stmt2->execute()) {
                    throw new Exception("Error inserting into trailer_movement: " . $stmt2->error);
                }
                $stmt2->close();
            }
        }
        ...
        if (!empty($trip3_costumer) || !empty($trip3_container) || !empty($trip3_status) || !empty($trip3_segment) || !empty($trip3_from) || !empty($trip3_to)) {
            $trip3_stmt = $conn->prepare("INSERT INTO trips 
                (d_id, trip_type, costumer, trip_container, trip_containerStat, container_activity, trip_haulingSegment, trip_haulingType, trip_from, trip_to, km_run, trip_status, trip_trailer, trip_genset) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'Active', ?, ?)");
            $trip3_stmt->bind_param("isssssssssdss", $dispatch_id, $trip3_type, $trip3_costumer, $trip3_container, $trip3_status, $trip3_activity, $trip3_segment, $trip3_hauling_type, $trip3_from, $trip3_to, $trip3_km_run, $trip3_trailer, $trip3_genset);
            $trip3_stmt->execute();
            $trip3_stmt->close();
            ...
            if (!empty($trip3_trailer)) {
                ...
                $stmt3->bind_param("ssssss", $trip3_trailer, $d_driverName, $location3, $recordedType3, $d_dispatcher, $d_datetime);
                if (!$stmt3->execute()) {
                    throw new Exception("Error inserting into trailer_movement: " . $stmt3->error);
                }
                $stmt3->close();
            }
        }
        ...
        if (!empty($trip4_costumer) || !empty($trip4_container) || !empty($trip4_status) || !empty($trip4_segment) || !empty($trip4_from) || !empty($trip4_to)) {
            $trip4_stmt = $conn->prepare("INSERT INTO trips 
                (d_id, trip_type, costumer, trip_container, trip_containerStat, container_activity, trip_haulingSegment, trip_haulingType, trip_from, trip_to, km_run, trip_status, trip_trailer, trip_genset) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'Active', ?, ?)");
            $trip4_stmt->bind_param("isssssssssdss", $dispatch_id, $trip4_type, $trip4_costumer, $trip4_container, $trip4_status, $trip4_activity, $trip4_segment, $trip4_hauling_type, $trip4_from, $trip4_to, $trip4_km_run, $trip4_trailer, $trip4_genset);
            $trip4_stmt->execute();
            $trip4_stmt->close();
            ...
            if (!empty($trip4_trailer)) {
                ...
                $stmt4->bind_param("ssssss", $trip4_trailer, $d_driverName, $location4, $recordedType4, $d_dispatcher, $d_datetime);
                if (!$stmt4->execute()) {
                    throw new Exception("Error inserting into trailer_movement: " . $stmt4->error);
                }
                $stmt4->close();
            }
        ...
        foreach ($containers as $container) {
            if (!empty($container)) {
                ...
                $checkStmt->bind_param("s", $container);
                $checkStmt->execute();
                ...
                if ($checkStmt->num_rows == 0) {
                    ...
                    $insertStmt->bind_param("s", $container);
                    $insertStmt->execute();
                    $insertStmt->close();
                }
                $checkStmt->close();
