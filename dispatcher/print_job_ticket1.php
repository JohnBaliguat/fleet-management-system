...
$startTimestamp = strtotime($data['da_timeIn']);

$hour   = (int)date('H', $startTimestamp);
$minute = (int)date('i', $startTimestamp);

// Determine pattern based on start minute
if ($minute == 0) {
    $minutePattern = 0;
} elseif ($minute == 31) {
    $minutePattern = 1;
...
// Target end time = 07:00 next day
$targetEnd = strtotime('07:00 +1 day', $startTimestamp);
...
while (true) {
    ...
    // Break if end time reached 07:00H
    if ($endHour == 7 && $endMin == 0) {
        break;
    }
